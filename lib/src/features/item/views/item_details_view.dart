...
class _ItemDetailsViewState extends ConsumerState<ItemDetailsView> {
  ...
  String _mainImageUrl = '', _name = '', _description = '';
  double _price = 0;
  ...
  Widget build(BuildContext context) {
    final itemList = ref.watch(itemDetailsControllerProvider);

    if (itemList.isNotEmpty) {
      _name = itemList[0].title ?? '';
      _description = itemList[0].description ?? '';
      // mainImageUrl = itemList[0].coverPhoto?.url ?? '';
      _price = itemList[0].price ?? 0;
    }

    print('ListItem ${itemList}');
